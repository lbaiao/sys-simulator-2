...
def plot_positions_and_actions(bs: base_station, mues: List[mobile_user], d2d_txs: List[d2d_user], d2d_rxs: List[d2d_user], actions_indexes: List[int]):
    bs_x = bs.position[0]
    bs_y = bs.position[1]
    mues_x = [i.position[0] for i in mues]
    mues_y = [i.position[1] for i in mues]
    ...
    coordinates = [(actions_indexes[i], d2d_txs_x[i], d2d_txs_y[i]) for i in range(len(d2d_txs))]
    for c in coordinates: plt.annotate(c[0], (c[1], c[2]))
...
def plot_positions_actions_pie(bs: base_station, mues: List[mobile_user], d2d_txs: List[d2d_user], d2d_rxs: List[d2d_user], actions_indexes: List[int], values: List[float], title: str, mue_success: int):
    bs_x = bs.position[0]
    bs_y = bs.position[1]
    mues_x = [i.position[0] for i in mues]
    mues_y = [i.position[1] for i in mues]
    ...
    p2 = ax1.plot(d2d_txs_x, d2d_txs_y, 'd', label='D2D TX')
    p3 = ax1.plot(d2d_rxs_x, d2d_rxs_y, 'd', label='D2D RX')    

    coordinates = [(f'({i},{actions_indexes[i]})', d2d_txs_x[i], d2d_txs_y[i]) for i in range(len(d2d_txs))]
    for c in coordinates: ax1.annotate(c[0], (c[1], c[2]))
    ...
    ax1.set_title(f'N={len(actions_indexes)}, M_S={bool(mue_success)}')
    ax1.legend()
