...
class FixedPositionsEnvironment:
    """
    Environment implemented for a Q Learning Based Power Control algorithm based on  
    ...
    (PIMRC) (pp. 1-6). IEEE.
    This environment agents states are based on their distance to the BS.
    """
    ...
    def get_state(self):
        ...
        n_zones = 4
        state = 99
        zones = [self.bs.radius*i/n_zones for i in range(n_zones+1)]
        for d in list(zip(*self.d2d_pairs))[0]:
            for i in range(n_zones):
                if d.distance_to_bs >= zones[i] and d.distance_to_bs < zones[i+1]:
                    state = i
            states.append(state)
        return states
