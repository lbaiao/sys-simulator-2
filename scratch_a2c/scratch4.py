...
def test(env: CompleteEnvironmentA2C2, framework: ActorCritic,
...
def run():
    ...
    env_params = EnvironmentParameters(
        ...
        bs_gain, user_gain, sinr_threshold_mue, n_mues, n_d2d,
        n_rb, bs_radius, c_param=C, mue_margin=mue_margin)

    reward_function = rewards.dis_reward_tensor2
    environment = CompleteEnvironmentA2C2(env_params, reward_function)

    framework = ActorCritic(environment.state_space_size,
                            environment.action_space_size,
                            HIDDEN_SIZE, mu, std)
    framework.load_state_dict(torch.load(f'{cwd}/models/a2c/script4.pt'))
