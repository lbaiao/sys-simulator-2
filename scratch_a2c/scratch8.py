...
from sys_simulator.a2c.a2c import ActorLSTMDiscrete, CriticMLP


def test(env: CompleteEnvironmentA2C2, actor: ActorLSTMDiscrete,
         critic: CriticMLP, max_d2d: int, num_episodes: int,
         episode_steps: int, aux_range: List[int], actions: List[float]):
    ...
    for ep in range(num_episodes):
        actor_h, actor_c = actor.initHidden()
...
def run():
    ...
    actor.load_state_dict(
        torch.load(f'{cwd}/models/a2c/script8_actor.pt')
    )
    critic.load_state_dict(
        torch.load(f'{cwd}/models/a2c/script8_critic.pt')
    )
    ...
    mue_spectral_effs, d2d_spectral_effs, bag, = \
        test(environment, actor, critic, MAX_NUMBER_OF_AGENTS,
             MAX_NUM_EPISODES, STEPS_PER_EPISODE, aux_range, actions)
