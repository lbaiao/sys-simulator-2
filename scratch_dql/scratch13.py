...
def test(env: DistanceEnvironmentMulti, policy: QTensor, max_d2d: int, num_episodes: int, episode_steps: int):
    ...
    for _ in range(num_episodes):
        ...
        interference_indicator = 1
        total_reward = 0.0
        i = 0
        while not done:                        
            for j, agent in enumerate(agents):
                    action_index = policy[(n_agents, obs[j], interference_indicator)]
            ...
            if i >= episode_steps:
                break
        mue_spectral_effs[n_agents].append(env.mue_spectral_eff)
        d2d_spectral_effs[n_agents].append(env.d2d_spectral_eff)
...
policy = np.load(f'{lucas_path}/models/script20.npy')

# policy 5 test
total_reward, mue_spectral_effs, d2d_spectral_effs, bag = test(environment, policy, MAX_NUMBER_OF_AGENTS, 50, 25)
