...
def test(env: CompleteEnvironment3, framework: ExternalDQNFramework, max_d2d: int, num_episodes: int, episode_steps: int):
    ...
    return total_reward, mue_spectral_effs, d2d_spectral_effs, bag, action_counts
...
framework.policy_net.load_state_dict(torch.load(f'{lucas_path}/models/script24.pt'))

reward_function = rewards.dis_reward_tensor

# policy 5 test
total_reward, mue_spectral_effs, d2d_spectral_effs, bag, action_counts_total = test(environment, framework, MAX_NUMBER_OF_AGENTS, 5000, 25)
# total_reward, mue_spectral_effs, d2d_spectral_effs, bag, action_counts_total = test(environment, framework, MAX_NUMBER_OF_AGENTS, 10, 5)
