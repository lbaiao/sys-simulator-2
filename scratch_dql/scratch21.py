#  Similar to scratch18.py, but the model is trained till 20 devices. We also measure how many times the devices
#  chose the same tx power. We use script26 model. We simulate for N_D2D in {10,15,20}
...
framework.policy_net.load_state_dict(torch.load(f'{lucas_path}/models/script26.pt'))

reward_function = rewards.dis_reward_tensor

# policy 5 test
aux_range = [10,15,20]
total_reward, mue_spectral_effs, d2d_spectral_effs, bag, action_counts_total, equals_counts_total = test(environment, framework, MAX_NUMBER_OF_AGENTS, 5000, 10, aux_range)
