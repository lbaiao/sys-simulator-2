...
def train(agents: List[ExternalDQNAgent], frameworks: List[ExternalDQNFramework], env: CompleteEnvironment, params: TrainingParameters):
            ...
            if i >= params.steps_per_episode:
                ...
                for j, agent in enumerate(agents):
                    if counts[j] < awaits[j]:
                        ...
                        agent.get_action(frameworks[j], obs[j])
                ...
                i += 1
                for j, agent in enumerate(agents):
                    frameworks[j].replay_memory.push(obs[j], actions[j], next_obs[j], rewards[j])
                    frameworks[j].learn()
                obs = next_obs
