    ...
    for episode in range(MAX_NUM_EPISODES):
        ...
        for i in range(STEPS_PER_EPISODE):
            for j, (agent, f) in enumerate(zip(agents, frameworks)):
                action, dist, value = agent.act_continuous(f, obs[j])
        ...
        for j, (agent, f) in enumerate(zip(agents, frameworks)):
            _, _, next_value_t = agent.act_continuous(f, next_obs_t[j])
...
def test(
    ...
    while True:
        ...
        for j, (agent, framework) in enumerate(zip(agents, frameworks)):
            action, _, _ = agent.act_continuous(framework, obs[j])
            agent.set_action(action)
            bag.append(action)
