...
# Everything is in dB. One NN for each agent.
import os
...
range_n_d2d = range(1, MAX_NUMBER_OF_AGENTS + 1, 1)
# more parameters
    ...
    for episode in range(MAX_NUM_EPISODES):
        ...
        for i in range(STEPS_PER_EPISODE):
            for j, agent in enumerate(agents):
                ...
                framework.log_probs[0][i] = log_prob
                framework.values[0][i] = value
                framework.entropy[0][i] = dist.entropy()
            # perform an environment step
            next_obs_t, rewards_t = env.step(agents)
            for r in rewards_t:
                framework.rewards[0][i] = r
