# Similar to script 6, but with LSTM.
# The actor learns only with 1 agent experience.
# The critic still learns with all agents experiences.
...
from sys_simulator.a2c.a2c import \
    ActorLSTMDiscrete, CriticLSTM, compute_gae_returns
...
def run():
    ...
    STEPS_PER_EPISODE = 10    # testing
    MAX_NUM_EPISODES = 2
    if os.environ[env_variables.MODE] == env_variables.MODE_PRODUCTION:
        STEPS_PER_EPISODE = 20
        MAX_NUM_EPISODES = 2700 * 1
    if os.environ[env_variables.MODE] == env_variables.MODE_TEST:
        STEPS_PER_EPISODE = 10    # testing
        MAX_NUM_EPISODES = 2      # testing
    ...
    actor = ActorLSTMDiscrete(environment.state_space_size,
                              NUM_ACTIONS, HIDDEN_SIZE)
    critic = CriticLSTM(environment.state_space_size, HIDDEN_SIZE)                              
    actor_optimizer = optim.Adam(actor.actor.parameters(), lr=LEARNING_RATE)
    critic_optimizer = optim.Adam(critic.critic.parameters(), lr=LEARNING_RATE)
    # training loop
    episode = 0
    d2d_spectral_effs = []
    mue_spectral_effs = []
    ...
    while episode < MAX_NUM_EPISODES:
        ...
        log_probs = torch.zeros((1, STEPS_PER_EPISODE)).to(device)
        values = torch.zeros((n_agents, STEPS_PER_EPISODE+1)).to(device)
        rewards = torch.zeros((n_agents, STEPS_PER_EPISODE)).to(device)
        entropy = torch.zeros((1, STEPS_PER_EPISODE)).to(device)
        i = 0
        done = False
        actor_hidden_h_list = []
        actor_hidden_c_list = []
        critic_hidden_h_list = []
        critic_hidden_c_list = []
        actor_hidden_h, actor_hidden_c = actor.init_hidden()
        critic_hidden_h, critic_hidden_c = critic.init_hidden()
        # actions = []  # used for debug purposes
        while not done and i < STEPS_PER_EPISODE:
            # agents choose their actions
            # actions_t = []  # used for debug purposes
            # first agent acts and its info is stored
            agent = agents[0]            
            action_index, dist, value, actor_hidden_h, actor_hidden_c,\
                critic_hidden_h, critic_hidden_c = agent.act_lstm_discrete(
                    actor, critic, obs[0], actor_hidden_h, actor_hidden_c,
                    critic_hidden_h, critic_hidden_c
                )
            agent.set_action(actions[action_index.item()])
            log_prob = dist.log_prob(action_index)
            log_probs[0][i] = log_prob
            values[0][i] = value
            entropy[0][i] = dist.entropy()
            # deal with the other agents
            if len(agents) > 0:
                for j, agent in enumerate(agents[1:]):
                    action_index, dist, value = \
                        agent.act_discrete(a2c, obs[j+1])
                    agent.set_action(actions[action_index.item()])
                    values[j+1][i] = value
