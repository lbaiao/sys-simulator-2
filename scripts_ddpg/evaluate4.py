...
PRINT_EVERY = TEST_NUM_EPISODES // 10
# env parameters
RND_SEED = True
SEED = 42
CHANNEL_RND = True
...
a_max = 60
a_offset = -10
framework = torch.load(MODELS_PATHS, map_location=torch_device)
...
if __name__ == '__main__':
    ...
    r = test(framework, surr_agents, start)
    print(f'avg mue avail: {np.mean(r["mue_availability"])}')
    print(f'avg d2d sinr: {np.mean(r["d2d_sinrs"], axis=0)}')
    # save stuff
    now = (time() - start) / 60
    data_file_path = f'{data_path}/log.pickle'
    save_with_pickle(r, data_file_path)
