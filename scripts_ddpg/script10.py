...
from sys_simulator.ddpg.agent import Agent, SurrogateAgent, SysSimAgent, SysSimAgentWriter
...
# continuous reward2 parameters
# JAIN_REWARD_PARAMETERS = {
#     'gamma1': 5.0,
#     'gamma2': 0.0,
#     'gamma3': 1.0
# }
# simple reward parameters
JAIN_REWARD_PARAMETERS = {
    'gamma1': 1.5,
    'gamma2': .1,
    'gamma3': 0.0
...
decaying_noise = DecayingGaussNoise(
    NORMAL_LOC, NORMAL_SCALE, NORMAL_T, NORMAL_MIN_SCALE, MAX_NUMBER_OF_AGENTS
)
central_agent = Agent(a_min, a_max, EXPLORATION,
...
def train(start: float, env: CompleteEnvironment12, writer: SummaryWriter):
    ...
    while step < MAX_STEPS:
        env.reset()
        env.build_scenario(
            surr_agents, d2d_limited_power=False,
            motion_model=MOTION_MODEL, pairs_distribution=PAIRS_DISTRIBUTION)
        ...
        while not done and i < STEPS_PER_EPISODE:
            ...
            for o, a, r, no in zip(obs, actions, rewards, next_obs):
                framework.replay_memory.push(o, a, r, no, done)
...
def test(framework: Framework, env):
...
def run():
    ...
    gen.save_with_pickle(env, f'{data_path}/env.pickle')
