# Same as scipt 7, but only one Q table exists and it is updated by all agents.
...
MAX_NUM_EPISODES = int(0.7/EPSILON_DECAY)
...
actions = [i*p_max/4/10 + 1e-9 for i in range(11)]
agents = [DistanceAgent(agent_params, actions) for i in range(n_d2d)] # 1 agent per d2d tx
q_table = QTable(len(actions)*2, len(actions), learn_params)
