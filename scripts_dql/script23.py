...
from sys_simulator.parameters.parameters import EnvironmentParameters, TrainingParameters, DQNAgentParameters, LearningParameters
from sys_simulator.q_learning import rewards
from sys_simulator.q_learning.q_table import DistributedQTable
from sys_simulator.dqn.dqn import DQN
from sys_simulator.dqn.replayMemory import ReplayMemory
from sys_simulator.dqn.externalDQNFramework import ExternalDQNFramework
from sys_simulator.dqn.agents.dqnAgent import ExternalDQNAgent
from sys_simulator.q_learning.environments.completeEnvironment2 import CompleteEnvironment2
    ...
    for episode in range(params.max_episodes):
        ...
        while not done:
            if i >= params.steps_per_episode:
                ...
                for j, agent in enumerate(agents):
                    framework.replay_memory.push(
                        obs[j], actions[j], next_obs[j], rewards[j])
                framework.learn()
                obs = next_obs
            ...
            mue_spectral_eff_bag.append(
                env.mue_spectral_eff)     # mue spectral eff
...
# SCRIPT EXEC
# training
mue_spectral_effs, d2d_spectral_effs = train(
    ext_framework, environment, train_params, agent_params, MAX_NUMBER_OF_AGENTS)
...
torch.save(ext_framework.policy_net.state_dict(),
