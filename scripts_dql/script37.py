# Similar to Script35, but with CompleteEnvironment8dB
...
from sys_simulator.q_learning.environments.completeEnvironment8dB \
    import CompleteEnvironment8dB
...
def calculate_interferences(env: CompleteEnvironment8dB):
    bs = env.bs
    mue = env.mue
...
def print_stuff(actions, env: CompleteEnvironment8dB):
...
def test():
    ...
    mue_success_rate = np.mean(
        np.array(mue_spectral_effs) > np.log2(1 + sinr_threshold_train)
    )
    # jain_index_avg = list()
