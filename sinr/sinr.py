...
def sinr_d2d(d2d_tx: d2d_user, d2d_devices: List[d2d_user], mue: mobile_user, noise_power: float, user_gain: float):
    d2d_tx_contrib = d2d_tx.tx_power * pathloss.pathloss_users(d2d_tx.distance_d2d/1000) * 2 * user_gain
    d2d_mue_distance = euclidean(d2d_tx.position, mue.position)
    mue_interference = mue.tx_power * pathloss.pathloss_users(d2d_mue_distance/1000)
    d2d_interferers = [d for d in d2d_devices if (d.id != d2d_tx.id and d.type == d2d_node_type.TX and d.rb == d2d_tx.rb)]
    d2d_interference = sum([d.tx_power * 2 * user_gain * pathloss.pathloss_users(euclidean(d2d_tx.position, d.position)/1000) for d in d2d_interferers])
