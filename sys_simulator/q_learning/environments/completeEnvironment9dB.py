...
class CompleteEnvironment9dB(RLEnvironment):
    """
    Similar to CompleteEnvironment9dB, but with different channels
    to the BS and to the devices. The channel is dynamic, and the
    devices have channel information, just before the transmission.
    """
    def __init__(
        self,
        ...
        **kwargs
        ...
        self.params = params
        super(CompleteEnvironment9dB, self).__init__(params,
    ...
    def calculate_d2d_pathlosses(self):
        for tx, rx in self.d2d_pairs:
            pathloss_to_rx = self.channel_to_devices.step(
                euclidean(tx.position, rx.position)
            )
            tx.set_pathloss_d2d(pathloss_to_rx)

    def calculate_mue_pathloss(self):
        pathloss_to_bs = self.channel_to_bs.step(
            euclidean(self.mue.position, self.bs.position)
        )
        self.mue.set_pathloss_to_bs(pathloss_to_bs)

    def calculate_all_pathlosses(self):
        if self.pathlosses_are_calculated:
            raise Exception(
                'Trying to calculate pathlosses more than once \
                in the same time step.'
            )
        self.calculate_d2d_pathlosses()
        self.calculate_mue_pathloss()
        self.calculate_d2d_pathlosses_to_interfering()
        self.calculate_mue_pathlosses_to_interfering()
        self.pathlosses_are_calculated = True

    def calculate_d2d_pathlosses_to_interfering(self):
        for tx, rx in self.d2d_pairs:
            d2d_interferers = [
            ...
            for itx, irx in d2d_interferers:
                pathloss = self.channel_to_devices.step(
                    euclidean(rx.position, itx.position)
                )
                pathlosses[itx.id] = pathloss
            distance_to_mue = euclidean(rx.position, self.mue.position)
            pathloss_to_mue = self.channel_to_devices.step(distance_to_mue)
            pathlosses[self.mue.id] = pathloss_to_mue
            tx.set_pathlosses_to_interfering(pathlosses)

    def calculate_mue_pathlosses_to_interfering(self):
        d2d_interferers = [
            p for p in self.d2d_pairs
        ...
        pathlosses = {}
        for itx, irx in d2d_interferers:
            pathloss = self.channel_to_bs.step(
                euclidean(itx.position, self.bs.position)
            )
            pathlosses[itx.id] = pathloss
    ...
    def sinr_mue(
        ...
        for d in d2d_interferers:
            interference = db_to_power(
                    ...
                    user_gain +
                    bs_gain -
                    mue.pathlosses_to_interfering[d.id]
                )
    ...
    def sinr_d2d(self, d2d_tx: d2d_user, d2d_rx: d2d_user,
        ...
        d2d_tx_contrib = d2d_tx.tx_power - d2d_tx.pathloss_d2d + 2 * user_gain
        # set the power at the receiver
        d2d_tx.set_power_at_receiver(d2d_tx_contrib)
        mue_interference = mue.tx_power - \
            d2d_tx.pathlosses_to_interfering[mue.id] + 2 * user_gain
        ...
        for d in d2d_interferers:
            interference = \
                db_to_power(
                    d.tx_power +
                    2 * user_gain -
                    d2d_tx.pathlosses_to_interfering[d.id]
                )
            d2d_interferences.append((d.id,  interference))
